...
sink("log/5_4.log.txt", split=TRUE)
...
tryCatch({
  ...
  train_ids =   sample(which(Pheno$tissue_type == "Breast"), ceiling(length(which(Pheno$tissue_type == "Breast")) * 0.5 ))
  ...
  df = data.frame(
    ...
    PA=rowMeans(abs(div$Mat.div)[, which(Groups=="BREAST")]),
