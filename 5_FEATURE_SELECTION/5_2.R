...
sink("log/5_2.log.txt", split=TRUE)
...
tryCatch({
  ...
  sel_tissues = setdiff(tissues, "Prostate")
  ...
  Groups = factor(ifelse(Pheno$tissue_type[-train_ids] == "Prostate", "PROSTATE", "OTHER"))
  ...
  save(div, train_ids, Groups, file="obj/5_2_data.rda")
  
  cat("Computing Chi-squared tests\n")
  
  C = computeChiSquaredTest(Mat=div$Mat.div, Groups=Groups, classes=c("PROSTATE", "OTHER"))  
  ...
  df = data.frame(
    ...
    PB=rowMeans(abs(div$Mat.div)[, which(Groups=="PROSTATE")]),
